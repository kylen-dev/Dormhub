...
class BookingController extends Controller
{
    ...
    public function landlordContracts(Request $request)
{
    $landlordId = auth()->id();

    $query = Booking::with(['tenant', 'property', 'room'])
        ->where('landlord_id', $landlordId);

    // ✅ Filter by contract_status if provided
    if ($request->has('contract_status') && $request->contract_status !== '') {
        $query->where('contract_status', $request->contract_status);
    }

    // ✅ Search by tenant name, property title, or email
